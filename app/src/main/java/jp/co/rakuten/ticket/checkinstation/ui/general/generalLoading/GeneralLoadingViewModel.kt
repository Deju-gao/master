...
@HiltViewModel
class GeneralLoadingViewModel @Inject constructor(private val apiRepository: ApiRepository) : BaseViewModel() {
    ...
    fun onViewResume(orderNo: String, ticketList: List<PrintedUpdateBody.PrintedTicketList>, generalActivity: GeneralActivity) {
        FirebaseLogUtil.getInstance().uploadPageLog(GeneralLoadingPage)
        val svgList = generalActivity.svgList
        ...
        for (newSvgList in svgList) {
            ...
            SaveDataUtil.getInstance().saveBitmapToLocal(generalActivity, bitmap)
            //print bitmap
            val target = generalActivity.intent.getStringExtra("printer_target")
            if (target?.let { BcpUtil.getInstance().checkBluetoothConnect(generalActivity, it) } == true) {
                ...
                bcpPrintTicket.setListener(object : BcpPrintTicket.BcpPrintCompleteListener {
                    override fun bcpPrintComplete(isSuccess: Boolean) {
                        if (isSuccess) {
                            updateTicket(orderNo, ticketList, generalActivity)
                        }
                    }
                })
                bcpPrintTicket.printMethod(1, target, generalActivity)
            }
        }
    }

    private fun updateTicket(orderNo: String, ticketList: List<PrintedUpdateBody.PrintedTicketList>, generalActivity: GeneralActivity) {
        apiRepository.printedUpdate(orderNo, ticketList)
            .execute(this@GeneralLoadingViewModel,
                onSuccess = {
                },
                onError = {
                    FirebaseLogUtil.getInstance().uploadApiException(GeneralLoadingPage.pageName, it)
                    when(it) {
