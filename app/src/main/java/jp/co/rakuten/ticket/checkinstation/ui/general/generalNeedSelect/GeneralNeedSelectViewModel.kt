...
import jp.co.rakuten.ticket.checkinstation.util.GeneralNeedSelectAllTicketButton
import jp.co.rakuten.ticket.checkinstation.util.GeneralNeedSelectPage
...
@HiltViewModel
class GeneralNeedSelectViewModel @Inject constructor(private val apiRepository: ApiRepository) : BaseViewModel() {
    ...
    //view resume
    fun onViewResume() {
        FirebaseLogUtil.getInstance().uploadPageLog(GeneralNeedSelectPage)
    }

    fun onUnneededSelectTargetCardDidTap(orderNo: String, context: Context, refreshMode: Boolean) {
        FirebaseLogUtil.getInstance().uploadPageLog(GeneralNeedSelectAllTicketButton)
        ticketList.clear()
        isLoadingVisible.postValue(true)
        apiRepository.qrTicketDataCollection(orderNo, refreshMode)
            .execute(this@GeneralNeedSelectViewModel,
                onSuccess = {
                    if (it.collection != null) {
                        ...
                        if (ticketList.isNotEmpty()) {
                            toVerifyPage.postValue(
                                GeneralVerifyArguments(
                                    orderNo = orderNo,
                ...
                onError = {
                    FirebaseLogUtil.getInstance().uploadApiException(GeneralNeedSelectPage.pageName, it)
                    when(it) {
