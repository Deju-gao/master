...
@HiltViewModel
class AllAgainViewModel @Inject constructor(private val apiRepository: ApiRepository) : BaseViewModel() {
    ...
    //view resume
    fun onViewResume() {
        FirebaseLogUtil.getInstance().uploadPageLog(AllInputPasswordPage)
    }

    //next button did tap
    fun onNextButtonDidTap(context: Context) {
        FirebaseLogUtil.getInstance().uploadPageLog(AllInputPasswordNextButton)
        ...
        apiRepository.login(LoginUtil.getInstance().getUserId(), number.value!!, LoginUtil.getInstance().getCompanyCode(), UUID.randomUUID().toString())
            .execute(this@AllAgainViewModel,
                onSuccess = {
                    if (it.code == 200) {
                ...
                onError = {
                    FirebaseLogUtil.getInstance().uploadApiException(AllInputPasswordPage.pageName, it)
                    when(it) {
